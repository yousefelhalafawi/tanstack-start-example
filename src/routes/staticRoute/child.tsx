...
export const Route = createFileRoute("/staticRoute/child")({
  component: StaticRouteChild,
});

function StaticRouteChild() {
  return (
      ...
      <div className="w-full max-w-2xl rounded-lg bg-[#252526] border border-[#3c3c3c] shadow-xl">
        ...
        <div className="flex items-center gap-2 px-4 py-2 border-b border-[#3c3c3c]">
          ...
          <span className="ml-3 text-sm text-gray-400 font-mono">
            src/routes/staticRoute/add.tsx
          </span>
        </div>
...
export const Route = createFileRoute('/staticRoute/add')({
  component: StaticRouteAdd,
})

// URL:
// /staticRoute/add

// This page renders independently
`}
        </pre>

        {/* Explanation */}
        <div className="border-t border-[#3c3c3c] px-4 py-3 text-sm font-mono text-emerald-400 bg-[#1e1e1e]">
          Explanation: This route does NOT depend on /staticRoute UI.
        </div>
      </div>
    </div>
  );
